...
import {
  ...
  submitGoodsReceiptForApproval, approveGoodsReceipt, rejectGoodsReceipt, postGoodsReceipt,
  fetchWarehouses, fetchItems,
  type GoodsReceipt, type Warehouse, type Item,
} from '@/api/tenant'
...
export default function GoodsReceiptsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => fetchGoodsReceipts({ page }).then((r) => r.data),
  })

  const { data: detail } = useQuery({
    queryKey: ['goods-receipt', detailId],
    queryFn: () => fetchGoodsReceipt(detailId!).then((r) => r.data.data),
    enabled: !!detailId,
  })
  ...
  const { register, handleSubmit, reset, setValue, watch, control, formState: { errors, isSubmitting } } =
    useForm<FormValues>({ resolver: zodResolver(schema), defaultValues: { lines: [{ item_id: '', quantity: 1, unit_cost: 0, batch_id: null }] } })

  const { fields, append, remove } = useFieldArray({ control, name: 'lines' })

  const openCreate = () => {
    reset({ warehouse_id: '', receipt_date: new Date().toISOString().slice(0, 10), reference: null, lines: [{ item_id: '', quantity: 1, unit_cost: 0, batch_id: null }] })
    setCreateOpen(true)
  }
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        {canEdit && <Button size="sm" onClick={openCreate}><Plus className="h-4 w-4 mr-1" /> New GRN</Button>}
      </div>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit((v) => create.mutate(v))} className="space-y-4 py-2">
            ...
            <div className="space-y-1">
              ...
              <Input {...register('reference')} placeholder="PO number or supplier ref" />
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button type="button" size="sm" variant="outline" onClick={() => append({ item_id: '', quantity: 1, unit_cost: 0, batch_id: null })}>
              ...
              <div className="space-y-2">
                {fields.map((field, i) => (
                  <div key={field.id} className="grid grid-cols-[1fr_80px_90px_80px_32px] gap-2 items-start">
                    ...
                    <Input type="number" step="0.01" placeholder="Unit cost" {...register(`lines.${i}.unit_cost`)} />
            ...
            {create.isError && <p className="text-xs text-destructive">Failed to create GRN</p>}
            <DialogFooter>
      ...
      <Dialog open={!!detailId} onOpenChange={(o) => !o && setDetailId(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>GRN Detail — {detail?.grn_number}</DialogTitle></DialogHeader>
          {detail && (
            <div className="space-y-4 py-2">
                ...
                <tbody>
                  {detail.lines?.map((l, i) => (
                    <tr key={i} className="border-t">
                      ...
                      <td className="px-3 py-2 text-right">{l.unit_cost}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <ApprovalActions
                ...
                onSubmit={(c) => submitGoodsReceiptForApproval(detail.id, c).then(() => qc.invalidateQueries({ queryKey: ['goods-receipt', detail.id] }))}
                onApprove={(c) => approveGoodsReceipt(detail.id, c).then(() => qc.invalidateQueries({ queryKey: ['goods-receipt', detail.id] }))}
                onReject={(c) => rejectGoodsReceipt(detail.id, c).then(() => qc.invalidateQueries({ queryKey: ['goods-receipt', detail.id] }))}
                onPost={() => postGoodsReceipt(detail.id).then(() => { qc.invalidateQueries({ queryKey: ['goods-receipt', detail.id] }); qc.invalidateQueries({ queryKey: ['goods-receipts'] }) })}
              />
            </div>
          )}
        </DialogContent>
